...
results_dir = 'results-exp3-errorbar13689-500reps'
checkpoints_dir = 'checkpoints-exp3-errorbar13689-500reps'
...
# Same as function in default.py, except we also record trial number when saving checkpoints
...
def train_and_predict(i: int, rep: int, ith_sample_X, sample_X, sample_Y, train_model_func, predict, trial):
    ...
    if train_model_func == train_model:
        ...
        reg = 0
    model.save_weights(
        f'./{checkpoints_dir}/checkpoint_sample{i}_trial{trial}_rep{rep}_i={scenario == 1}_reg={reg == 1}.weights.h5')
...
# Exact same function as in target.py
...
if __name__ == "__main__":
    print("Start process for errorbar ...")
    ...
    for trial in range(trials):
        ...
        np.savetxt(
            f"./{results_dir}/sample{i}_trial{trial}_predict_i.csv", predict_i, delimiter=",")
        np.savetxt(
            f"./{results_dir}/sample{i}_trial{trial}_predict_noi.csv", predict_noi, delimiter=",")
        np.savetxt(
            f"./{results_dir}/sample{i}_trial{trial}_predict_i_reg.csv", predict_i_reg, delimiter=",")
        np.savetxt(
            f"./{results_dir}/sample{i}_trial{trial}_predict_noi_reg.csv", predict_noi_reg, delimiter=",")
        # Save epsilon values
        ...
        np.savetxt(
            f"./{results_dir}/sample{i}_trial{trial}_epsilons.csv", epsilons, delimiter=",")
        np.savetxt(
            f"./{results_dir}/sample{i}_trial{trial}_epsilons_reg.csv", epsilons_reg, delimiter=",")
